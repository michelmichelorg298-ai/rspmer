...
import { ventesApi, fournituresApi, productsApi } from '../api/client.js'
...
export default function Ventes() {
  ...
  const recentEntries = [...items]
    .sort((a, b) => new Date(b.created_at || b.date_livraison || 0) - new Date(a.created_at || a.date_livraison || 0))
    .slice(0, 6)

  const todaysCount = items.filter(item => (item.date_livraison || item.created_at || '').slice(0, 10) === today).length
  ...
  const dailyCounts = Array.from({ length: 7 }).map((_, index) => {
    ...
    const count = items.filter(item => (item.date_livraison || item.created_at || '').slice(0, 10) === iso).length
  ...
  const save = async (e) => {
    ...
    try {
      const payload = {
        ...
        date_obtenu_fourniture: form.date_obtenu_fourniture || null,
        kilo: Number(form.kilo || 0),
        prix: Number(form.prix || 0),
      }
      await ventesApi.create(payload)
      push(toasts, setToasts, 'success', 'Vente ajoutée en base')
      setForm(emptyForm)
      setOpen(false)
      load()
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 mt-1">Saisie de vente, nombre de saisies par jour et aperçu des dernières saisies</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="card p-4">
          ...
          <p className="mt-2 text-lg font-bold text-slate-800">{lastEntry ? lastEntry.nom_ou_restaurant : 'Aucune'}</p>
          <p className="text-sm text-slate-500">{lastEntry ? dateFr(lastEntry.date_livraison || lastEntry.created_at) : '—'}</p>
        </div>
      </div>
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Suivi des ventes sur les 7 derniers jours</p>
          </div>
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Dernières ventes enregistrées en base</p>
          </div>
            ...
            <div key={item.id} className="flex items-start justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-semibold text-slate-800">{item.nom_ou_restaurant}</p>
                </div>
                <p className="text-xs text-slate-500">{item.nom_produit_texte} • {kg(item.kilo)} • {euro(item.kilo * item.prix)}</p>
              </div>
              <span className="text-[11px] font-medium text-slate-500 whitespace-nowrap">{dateFr(item.date_livraison || item.created_at)}</span>
            </div>
          ))}
        </div>
      </div>

      <Modal open={open} onClose={() => setOpen(false)} title="Nouvelle vente" size="lg">
        <form onSubmit={save} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="input-label">Date de livraison *</label>
              <input required type="date" value={form.date_livraison} onChange={e => setForm({ ...form, date_livraison: e.target.value })} className="input" />
            </div>
            <div>
              <label className="input-label">Client / restaurant *</label>
              <input required value={form.nom_ou_restaurant} onChange={e => setForm({ ...form, nom_ou_restaurant: e.target.value })} className="input" />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.fourniture_id} onChange={e => setForm({ ...form, fourniture_id: e.target.value })} className="input">
            ...
            <div>
              ...
              <input value={form.lieu} onChange={e => setForm({ ...form, lieu: e.target.value })} className="input" placeholder="Ex : Port-Louis" />
            </div>
            <div>
              <label className="input-label">Quantité (kg) *</label>
              <input required type="number" step="0.01" min="0" value={form.kilo} onChange={e => setForm({ ...form, kilo: e.target.value })} className="input" />
            </div>
            <div>
              <label className="input-label">Prix unitaire (€ / kg) *</label>
              <input required type="number" step="0.01" min="0" value={form.prix} onChange={e => setForm({ ...form, prix: e.target.value })} className="input" />
            </div>
            <div className="md:col-span-2">
              <label className="input-label">Téléphone</label>
              <input value={form.numero_telephone} onChange={e => setForm({ ...form, numero_telephone: e.target.value })} className="input" placeholder="Ex : 034 00 000 00" />
            </div>
          </div>
